# tic-tac-toe player that plays perfectly as x, but can be trapped as o by the play 1 - 5 - 9 - 3 - 7 etc

from BasePlayer import BasePlayer
import random
from ttt_utility import find_moves, can_trap, can_win

class BeatablePlayer(BasePlayer):
    def __init__(self):
        pass
    ...
    def get_move(self, board, cur_player):
        ...
        if cur_player == 1: # x's turn
            ...
            # don't block one particular trap!
            if b.count(' ') == 6:
                if b[4] == 'o' and ((b[0] == 'x' and b[8] == 'x') or (b[2] == 'x' and b[6] == 'x')): # particular trap
                    return random.choice([i for i in find_moves(b) if i in [0, 2, 6, 8]]) # random valid corner
